...
@section('right-box')
                ...
                @unless(!$res)
                    @foreach($res as $vo)
                        <tr>
                            ...
                            <td data="{{$vo['id']}}"><a href="{{ url('admin/editToupiao').'/'.$vo['id'] }}">修改</a> <a href="{{ url('admin/toupiaoRes').'/'.$vo['id'] }}">查看结果 </a> @if($vo['status'] == 0)<a  href="{{ url('admin/toupiao/close',['id'=>$vo['id']]) }}" >关闭投票</a>@else<a  href="{{ url('admin/toupiao/open',['id'=>$vo['id']]) }}" >开启投票</a>@endif </td>
                        </tr>
                    @endforeach
                @endunless
                </tbody>
    ...
    <script>
        ...
        @if (session('editres'))
            layer.alert('修改成功！');
        @endif
        @if (session('is_toupiao'))
            layer.alert('已有人投票，不可修改，请关闭重建！');
        @endif
