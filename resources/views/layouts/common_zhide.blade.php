        ...
        <div class="worth-main">
            @if(!empty($list_toupiao))
                @foreach($list_toupiao as $vo)
                    <div class="worth-list flex-justify" @if($usertype == 'person')onclick="location.href='{{ url('home/toupiaodetail',['id'=>$vo -> id]) }}'" @else onclick="noReg()" @endif >
            ...
            @if(!empty($list_article))
                @foreach($list_article as $vo)
                    <div class="worth-list flex-justify" @if($usertype == 'person') onclick="location.href='{{url('home/zhidedetail')}}/{{$vo -> id}}'" @else onclick="noReg()" @endif >
                        <span>{{$vo -> title}}@if($vo -> ishot == 1)<img src="{{asset('images/hot.png')}}" />@endif</span>
                    </div>
                @endforeach
            @endif
